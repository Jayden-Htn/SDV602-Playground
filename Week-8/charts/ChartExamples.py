...
def line_plot(**kwargs):
    ...
    """
    plt.plot([-1, -4.5, 16, 23]) # values for Y axis along time (X axis)
...
def discrete_plot(**kwargs):
    ...
    Args 
          **kwargs lets you pass arguments into this function
    """
    plt.plot([-1, -4.5, 16, 23], "ob") # ob = blue circle markers
    # See formating here :
...
def names_labels(**kwargs):
    ...
    Args 
          **kwargs lets you pass arguments into this function
    """
    days = range(1, 9) # days 1 to 8
    celsius_values = [25.6, 24.1, 26.7, 28.3, 27.5, 30.5, 32.8, 33.1]

    fig, ax = plt.subplots() # Challenge - Why "fig" here"

    ax.plot(days, celsius_values) # plot x and y values
...
def multiple_plots(**kwargs):
    ...
    ax.plot(days, celsius_min, "oy") # o = circle, y = yellow
    ax.plot(days, celsius_max)
    ax.plot(days, celsius_max, "or") # o = circle, r = red
...
def bar_chart(**kwargs):
    ...
    plt.bar(years, visitors, color="green") # bar chart
...
def histogram(**kwargs):
    ...
    gaussian_numbers = np.random.normal(size=10000) # 10,000 random numbers
    
    plt.hist(gaussian_numbers, bins=20) # 20 bins (bars) in the histogram
...
def scatter_plots(**kwargs):
    ...
    y1 = np.random.randint(2, 7, (11,)) # 11 random numbers between 2 and 7
    y2 = np.random.randint(9, 14, (11,))
    y3 = np.random.randint(15, 25, (11,))

    # Markers: https://matplotlib.org/api/markers_api.html

    plt.scatter(x, y1) # default marker is a circle
    plt.scatter(x, y2, marker='v', color='r') # v = triangle_down, r = red
    plt.scatter(x, y3, marker='^', color='m') # ^ = triangle_up, m = magenta
...
def pie_chart1(**kwargs):
    ...
    # autopct = auto percent, startangle = angle of first slice
    # explode = makes a slice pop out
